...
- **app/** — ページ固有のコンポーネント・hooks を `_components/` `_hooks/` にコロケート。機能の追加・削除がディレクトリ単位で完結するため、不要になったコードがそのまま残るリスクを減らせる。`/` は `/search` にリダイレクトし、app/ 直下にページコンポーネントを置かないことで見通しを良くしている
